...
from ..main_forms import EditAccountInfoForm
from ..base import BaseProcesser, BaseProcessersManager, EarlyStopProcessException
from controller import AccountManager


class EditAccountInfoProcesser(BaseProcesser[None]):
    def main_process(self, inner_dict: Dict[str, Any]) -> None:
        form: EditAccountInfoForm = inner_dict["form"]
        inner_dict["response"] = AccountManager.edit_info(
...
class EditAccountInfoProcesserManager(BaseProcessersManager):
    def pre_process_for_starting(self, **kwargs) -> Tuple[Dict[str, Any], Dict[str, Any]]:
        ...
        try:
            inner_dict = {}
            inner_dict["form"] = EditAccountInfoForm.init_from_dict(kwargs=kwargs)
        except:
